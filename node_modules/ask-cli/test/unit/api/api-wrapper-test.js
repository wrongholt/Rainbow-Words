...
describe('api api-wrapper testing', () => {
    ...
    describe('# get-beta-test', () => {
        it('| will finally make a request to get beta test', () => {
            apiWrapper.callGetBetaTest('skill-id', TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest')).equal(true);
        });
    });

    describe('# create-beta-test', () => {
        it('| will finally make a request to create beta test', () => {
            apiWrapper.callCreateBetaTest('skill-id', "feedbackEmail", TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest')).equal(true);
        });

        it('| will finally make a request to create beta test without feedbackEmail', () => {
            apiWrapper.callCreateBetaTest('skill-id', null, TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest')).equal(true);
        });
    ...
    describe('# update-beta-test', () => {
        it('| will finally make a request to update beta test', () => {
            apiWrapper.callUpdateBetaTest('skill-id', "feedbackEmail", TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest')).equal(true);
        });

        it('| will finally make a request to update beta test without feedbackEmail', () => {
            apiWrapper.callUpdateBetaTest('skill-id', null, TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest')).equal(true);
        });
    ...
    describe('# start-beta-test', () => {
        it('| will finally make a request to start beta test', () => {
            apiWrapper.callStartBetaTest('skill-id', TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest/start')).equal(true);
        });
    });

    describe('# end-beta-test', () => {
        it('| will finally make a request to end beta test', () => {
            apiWrapper.callEndBetaTest('skill-id', TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest/end')).equal(true);
        });
    });

    describe('# list-beta-testers', () => {
        it('| will finally make a request to list testers in beta test', () => {
            apiWrapper.callListBetaTesters('skill-id', null, null, TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes(`/skills/skill-id/betaTest/testers?maxResults=${CONSTANTS.BETA_TEST.DEFAULT_LIST_MAX_RESULT}`)).equal(true);
        });

        it('| will finally make a request to list testers in beta test with max results', () => {
            const maxResults = 100;
            apiWrapper.callListBetaTesters('skill-id', null, maxResults, TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes(`/skills/skill-id/betaTest/testers?maxResults=${maxResults}`)).equal(true);
        });

        it('| will finally make a request to list testers in beta test with token', () => {
            const nextToken = "nextToken";
            apiWrapper.callListBetaTesters('skill-id', nextToken, null, TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes(`/skills/skill-id/betaTest/testers?nextToken=${nextToken}&maxResults=${CONSTANTS.BETA_TEST.DEFAULT_LIST_MAX_RESULT}`))
                .equal(true);
        });

        it('| will finally make a request to list testers in beta test with token and max results', () => {
            const maxResults = 100;
            const nextToken = "nextToken";
            apiWrapper.callListBetaTesters('skill-id', nextToken, maxResults, TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url
                .includes(`/skills/skill-id/betaTest/testers?nextToken=${nextToken}&maxResults=${maxResults}`))
                .equal(true);
        });
    });

    describe('# add-beta-testers', () => {
        it('| will finally make a request to add beta testers', () => {
            apiWrapper.callAddBetaTesters('skill-id', [{emailId:"a@a.com"}], TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest/testers/add')).equal(true);
        });
    });

    describe('# remove-beta-testers', () => {
        it('| will finally make a request to remove beta testers', () => {
            apiWrapper.callRemoveBetaTesters('skill-id', [{emailId:"a@a.com"}], TEST_PROFILE, false, emptyCallback);
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest/testers/remove')).equal(true);
        });
    });

    describe('# send-reminder-to-beta-testers', () => {
        it('| will finally make a request to send reminder to beta testers', () => {
            apiWrapper.callSendReminderToBetaTesters(
                'skill-id',
                [{emailId:"a@a.com"}],
                TEST_PROFILE,
                false,
            ...
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest/testers/sendReminder')).equal(true);
        });
    });

    describe('# request-feedback-from-beta-testers', () => {
        it('| will finally make a request to request feedback from beta testers', () => {
            apiWrapper.callRequestFeedbackFromBetaTesters(
                'skill-id',
                [{emailId: "a@a.com"}],
                TEST_PROFILE,
                false,
            ...
            expect(oauthWrapper.tokenRefreshAndRead.calledOnce).equal(true);
            const param = oauthWrapper.tokenRefreshAndRead.getCall(0).args[0];
            expect(param.url.includes('/skills/skill-id/betaTest/testers/requestFeedback')).equal(true);
        });
    });
});
