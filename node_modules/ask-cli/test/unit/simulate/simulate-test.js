        ...
        describe('make all the way through to make a apiWrapper call', () => {
            beforeEach(() => {
                ...
                sandbox.stub(apiWrapper, 'callSimulateSkill')
                    .callsArgWith(6, {"body": {"id": 6789, "status": "IN_PROGRESS"}});
            });
            it ('| polling successful', () => {
                sandbox.stub(apiWrapper, 'callGetSimulation')
                    .callsArgWith(4, {"body": {"id": 6789, "status": "SUCCESSFUL"}});
            ...
            it ('| polling failed because the response does not have status field', () => {
                ...
                expect(console.error.getCall(1).args[0]).equal(
                    '𐄂 Unable to get skill simulation result for simulation id: 6789'
                );
            });

            it ('| polling failed because the response has invalid status', () => {
                sandbox.stub(apiWrapper, 'callGetSimulation')
                    .callsArgWith(4, {"body": {"id": 6789, "status": "INVALID"}});
                let clock = sandbox.useFakeTimers();
                ...
                expect(console.error.getCall(1).args[0]).equal(
                    '𐄂 [Error]: Invalid response for skill simulation'
                );
            });
        });
    });
